...
app.use(session({
    ...
    cookie: { secure: true, sameSite: 'lax' } // Cambia secure a true si usas HTTPS
}));
...
passport.use(new GoogleStrategy({
    ...
    callbackURL: "https://simon-dice.com/auth/google/callback"
...
const requireAuth = (req, res, next) => {
    if (req.session && req.session.userId) {
        ...
        res.redirect('https://simon-dice.com/');
    }
};
...
app.get('/juego', requireAuth, noCache, (req, res) => {
...
app.get('/auth/google/callback', passport.authenticate('google', {failureRedirect: '/'}), (req, res) => {
    ...
    res.redirect('/juego'); // Redirigir a la página principal del juego
});
...
app.post('/register', async (req, res) => {
    ...
    if (
        ...
        !validator.isEmail(email) // Validar formato de correo electrónico
    ) {
    ...
    try {// Hash de la contraseña, usando bcrypt
        ...
        await pool.query('INSERT INTO usersjuego (nombre, correo, password) VALUES (?, ?, ?)', [username, email, hashedPassword]);
...
// Iniciar el servidor
app.listen(port, () => {
    console.log(`Servidor del Juego corriendo en https://simon-dice.com/`);
});