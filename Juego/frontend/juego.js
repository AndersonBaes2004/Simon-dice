const API_URL = "https://simon-dice.com/juego"; // Cambia según tu URL del backend
...
function gameover() {
    ...
    if (monedas <= 0) {
        ...
        document.getElementById('SalirUser').onclick = function() {
            ...
            .then(() => window.location.replace("https://simon-dice.com/"));
        };
        ...
        .then(data => {
            if (data.success && data.name) {
                ...
                .then(res => {
                    ...
                    if (res.status === 401) {
                        window.location.replace("https://simon-dice.com/");
   ...
   // 5. Configurar el botón de cerrar sesión
    document.getElementById('logoutButton').addEventListener('click', function() {
        ...
        .then(data => {
            if (data.success) {
                localStorage.removeItem('username');
                window.location.replace("https://simon-dice.com/"); // Usar replace en vez de href y esto evita que el usuario vuelva a la página anterior al hacer clic en "Atrás"
             } else {
